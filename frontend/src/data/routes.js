...
export const findRoutes = (origin, destination) => {
  return busRoutes.filter(route => {
    const originMatch = route.origin.toLowerCase().includes(origin.toLowerCase()) ||
                       route.stops.some(stop => stop.name.toLowerCase().includes(origin.toLowerCase()));
    const destMatch = route.destination.toLowerCase().includes(destination.toLowerCase()) ||
                     route.stops.some(stop => stop.name.toLowerCase().includes(destination.toLowerCase()));
    return originMatch && destMatch;
  });
};

// Function to get reverse route
export const getReverseRoute = (routeId) => {
  ...
  return {
    ...route,
    id: `${routeId}-reverse`,
    name: `${route.destination} - ${route.origin}`,
    origin: route.destination,
    destination: route.origin,
    stops: [...route.stops].reverse().map((stop, index) => ({
      ...stop,
      order: index + 1
    })),
